        ...
        describe("getMetadata", () => {
          ...
          describe("when valid credentials are provided", () => {
            ...
            describe("search paging contract", () => {
              // searchResult() hardcoded index: 0 and ignored the requested count entirely:
              // measured live, Sonos asked for count=2 and got count=20/total=20. total stays the
              // number we HOLD (upstream search3 caps at 20 per category) so any total-driven
              // affordance in the app sees the same number it sees today; only the surplus items
              // stop being sent.
              it("slices to the requested count and reports the true total", async () => {
                const hits = Array.from({ length: 20 }, () => anAlbumSummary());
                musicLibrary.searchAlbums.mockResolvedValue(hits);
              ...
              it("echoes a non-zero index and returns that slice", async () => {
                const hits = Array.from({ length: 20 }, (_, i) =>
                  anAlbumSummary({ name: `Album ${i}` })
                );
                musicLibrary.searchAlbums.mockResolvedValue(hits);
              ...
              it("an index past the end is an empty page, not a wrap", async () => {
                musicLibrary.searchAlbums.mockResolvedValue(
                  Array.from({ length: 20 }, () => anAlbumSummary())
                );
              ...
              // nothing, which looks exactly like "no results" rather than like a bug. Missing
              // paging must mean "everything held", which is today's behaviour.
              ...
              it("treats count 0 as everything rather than as an empty page", async () => {
                musicLibrary.searchAlbums.mockResolvedValue(
                  Array.from({ length: 20 }, () => anAlbumSummary())
                );
                ...
                expect((result[0] as any).searchResult.count).toEqual(20);
              });
            });
